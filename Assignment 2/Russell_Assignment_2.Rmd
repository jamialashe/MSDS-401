---
title: 'Programming with R Assignment #2b (75 points)'
output:
...
bp1 <- data.frame(Outcomes=seq(0,10), Probabilities=dpois(0:10, .05*100))
bp2 <- data.frame(Outcomes=seq(0,10), Probabilities=dbinom(0:10, 100, .05))

bpx1 <- ggplot(bp1, aes(x=Outcomes, y=Probabilities)) + 
  geom_bar(stat="identity", fill="pink") +
  ggtitle("Poisson Probabilities") +
  theme(legend.position = "none")

bpx2 <- ggplot(bp2, aes(x=Outcomes, y=Probabilities)) + 
  geom_bar(stat="identity", fill="darkgreen") +
  ggtitle("Binomial Probabilities") +
  theme(legend.position = "none")

grid.arrange(bpx1, bpx2, nrow=1)
```
...
plot(b,prob,pch=22,main = "Cumlative Probability & Corresponding Outcomes",xlab="Corresponding Outcomes",ylab="Cumlative Probability")
points(3,0.867,col="red",pch=16)
text(x=3, y=0.92, col="white", labels="Median of Distribution = (3, 0.867)")
...
hist(faithful$waiting, main = "Occurences & Waiting Times", xlab ="Waiting Time", ylab = "Occurences", col ="gold")

w70 <- subset(faithful, waiting > 70)
e3 <- subset(w70, w70$eruptions < 3)
cond_prob <- nrow(e3)/nrow(w70)
cond_prob
...
w905 <- subset(faithful, faithful$waiting > 00 & faithful$eruptions < 5.0)
w905

ggplot(faithful, aes(x = waiting, y = eruptions)) + 
  geom_point(size = 1, color="black") + 
  ggtitle("Eruption Time & Waiting Time") +
  xlab("Waiting Time") +
  ylab("Eruption Time") +
  geom_hline(yintercept=3, color="yellow", linetype=2) +
  geom_vline(xintercept=70, color="yellow", linetype=2) +
  geom_point(x=71, y=2.383, size=1, color="blue") +
  geom_text(x=76, y=2.383, label="(71, 2.383)", color="blue")   
```

(ii)  What does the plot suggest about the relationship between eruption time and waiting time?

***Answer: The plot shows that as wait time increases the length of eruption also increases.    ***
...
wtime<-matrix(faithful$waiting, ncol=2, byrow="TRUE")
wtime

plot(main="Column 1 & 2 Wait Time",y=wtime[,2],x=wtime [,1],xlab="Wait Time - Column 1)",ylab = "Wait Time - Column 2)", col="pink")
...
cor.test(x=wtime[,1], y=wtime[,2], method="kendall")
```
...
weightlist <- ChickWeight$Time == 21 & (ChickWeight$Diet == "1" | ChickWeight$Diet == "3")

wlist <- subset(ChickWeight[weightlist, ], select = c(weight, Time, Chick, Diet))
wlist$Diet <- factor(wlist$Diet)
str(wlist)
...
par(mfrow = c(1,2))
bxp1 <- subset(wlist, Diet==1)
bxp2 <- subset(wlist, Diet==3)
boxplot(bxp1$weight, main="Weights & Diet 1", ylab="Weight", col="pink")
boxplot(bxp2$weight, main="Weights & Diet 3", ylab="Weight", col="darkgreen")
...
t.test(bxp1$weight, bxp2$weight, conf= 0.95, alternative="two.sided", var.equal= TRUE)
...
data(ChickWeight)
dwlist <- ChickWeight$Diet == "3"
before <- subset(ChickWeight[dwlist,], Time == 20, select =weight)$weight
after <- subset(ChickWeight[dwlist,], Time == 21, select =weight)$weight
cbind(before,after)
...
plot(before,after, main= "Chicks Weight Gain", xlab="Day 20", ylab="Day 21")
abline(a=0, b=1, h=NULL, V=NULL, reg=NULL, col="pink")
...
cinv <- cval*(sd/sqrt(n))
con <- mean-cinv
cat("One-Sided Confidence Interval: (", con, ", oo)\n")
```
...
par(mfrow = c(1, 2))
qqnorm(Nile, col="pink", main="Nile River Flow", cex=.75)
qqline(Nile)
boxplot(Nile, horizontal=FALSE, notch=TRUE, col="pink", main="Nile River Flow", ylab="River Flow'")
```
...
colnames(samp_sum) <- c("Mean", "Standard Deviation", "Variance")
...
par(mfrow = c(1, 2))
hist(sample1, freq=FALSE, xlim=c(750, 1050), ylim=c(0,0.025), col="pink")
curve(dnorm(x, mean = mean(sample1), sd = sd(sample1)), col = "darkgreen", lwd = 2, add = TRUE)
hist(sample2, freq=FALSE, xlim=c(750, 1050), ylim=c(0,0.025), col="pink")
curve(dnorm(x, mean = mean(sample2), sd = sd(sample2)), col = "darkgreen", lwd = 2, add = TRUE)
...
data(warpbreaks, package = "datasets")
str(warpbreaks)
mb <- median(warpbreaks$breaks)
hist(warpbreaks$breaks, main="Histogram of Loom Warp Breaks", xlab="Warp Breaks per Loom")
abline(v = mb, col = "darkblue")
text(x=37, y=12,"Median = 26", col="darkblue")

med <- factor(ifelse(warpbreaks$breaks < mb, "below", "above"))
warpbreaks <- cbind(warpbreaks, med)
summary(warpbreaks)
hh <- table(warpbreaks$tension, warpbreaks$med)
hh
...
```
```{r}
#Null hypothesis must be rejected. Variables are not independent.
```
...
totals <- table(warpbreaks$tension, warpbreaks$med)
mar_tbl <- addmargins(totals)
